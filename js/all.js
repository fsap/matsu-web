/*1430778707,,JIT Construction: v1720163,ja_JP*/
...
try {
    window.FB || (function(window) {
        ...
        __d("JSSDKXDConfig", [], {
            ...
            "XdBundleUrl": "\/connect\/xd_arbiter\/6p3So0GhyQP.js?version=41",
            "Flash": {
        ...
        __d("ApiClient", ["ArgumentError", "Assert", "CORSRequest", "FlashRequest", "flattenObject", "JSONPRequest", "Log", "ObservableMixin", "QueryString", "sprintf", "sdk.URI", "UrlMap", "ApiClientConfig", "invariant"], function(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t) {
            b.__markCompiled && b.__markCompiled();
            var u, v, w, x = l.MAX_QUERYSTRING_LENGTH, y = {
                get: true,
                post: true,
                'delete': true,
                put: true
                ...
                stream_get: true,
                users_getinfo: true
            }, aa = [], ba = [], ca = null, da = 50, ea = 105440539523;
            function fa(oa, pa, qa, ra) {
                if (w)
                    qa = ES('Object', 'assign', false, {}, w, qa);
                ...
                for (var ua = 0; ua < ta.length; ua++) {
                    var va = sa[ta[ua]], wa = ES('Object', 'assign', false, {}, qa);
                    if (va.execute(oa, pa, wa, ra))
                        return;
                }
                ra({
                    error: {
            ...
            function ga(oa, pa, qa, ra, sa, ta) {
                if (ta && ta.error)
                    ma.inform('request.error', pa, qa, ra, ta, ES('Date', 'now', false) - sa);
            ...
            function ia() {
                for (var oa = [], pa = 0, qa = arguments.length; pa < qa; pa++)
                    oa.push(arguments[pa]);
                var ra = ha(oa), sa = ra.uri, ta = ra.callback, ua = ra.params, va = ua.method;
                if (na(sa, va))
                    va = 'post';
                var wa = sa.getProtocol() && sa.getDomain() ? sa.setQueryData({}).toString(): r.resolve('graph') + sa.getPath();
            ...
            function ja() {
                for (var oa = [], pa = 0, qa = arguments.length; pa < qa; pa++)
                    oa.push(arguments[pa]);
                var ra = ha(oa), sa = ra.uri, ta = ra.callback, ua = ra.params, va = ua.method, wa = {
                    method: va,
                    relative_url: sa.removeQueryData('method').toString()
                };
                if (va.toLowerCase() == 'post') {
                    wa.body = o.encode(sa.getQueryData());
                    wa.relative_url = sa.setQueryData({}).toString();
                }
                aa.push(wa);
                ba.push(ta);
                if (aa.length == da) {
            ...
            function ka() {
                ...
                ca = null;
                if (oa.length === 1) {
                    var qa = oa[0], ra = pa[0], sa = qa.body ? o.decode(qa.body): null;
                ...
                ia('/', 'POST', {
                    ...
                    if (ES('Array', 'isArray', false, ta)) {
                        ...
                        ES(pa, 'forEach', true, function(ua) {
                            return ua({
                                error: {
            ...
            function la(oa, pa) {
                ...
                if (!pa)
                    m.warn('No callback passed to the ApiClient');
                var qa = oa.method.toLowerCase().replace('.', '_');
                oa.format = 'json-strings';
                oa.api_key = v;
                var ra = qa in z ? 'api_read': 'api', sa = r.resolve(ra) + '/restserver.php', ta = ES(ga, 'bind', true, null, pa, '/restserver.php', 'get', oa, ES('Date', 'now', false));
                fa(sa, 'get', oa, ta);
            }
            ...
            function na(oa, pa) {
                return (oa.toString().length > x && pa === 'get');
            }
        ...
        __d("sdk.Dialog", ["sdk.Canvas.Environment", "sdk.Content", "sdk.DOM", "DOMEventListener", "ObservableMixin", "sdk.Runtime", "Type", "sdk.UA", "sdk.fbt", "sdk.feature"], function(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p) {
            ...
            var w = m.extend({
                ...
                _createMobileLoader: function() {
                    var y = n.nativeApp() ? '': ('<table>' + '  <tbody>' + '    <tr>' + '      <td class="header_left">' + '        <label class="touchable_button">' + '          <input type="submit" value="' + o._("\u30ad\u30e3\u30f3\u30bb\u30eb") + '"' + '            id="fb_dialog_loader_close"/>' + '        </label>' + '      </td>' + '      <td class="header_center">' + '        <div>' + '         ' + o._("\u8aad\u307f\u8fbc\u307f\u4e2d...") + '        </div>' + '      </td>' + '      <td class="header_right">' + '      </td>' + '    </tr>' + '  </tbody>' + '</table>');
                    return x.create({
    ...
    new Image().src = "http:\/\/www.facebook.com\/" + 'common/scribe_endpoint.php?c=jssdk_error&m=' + encodeURIComponent('{"error":"LOAD", "extra": {"name":"' + e.name + '","line":"' + (e.lineNumber || e.line) + '","script":"' + (e.fileName || e.sourceURL || e.script) + '","stack":"' + (e.stackTrace || e.stack) + '","revision":"1720163","message":"' + e.message + '"}}');
}
